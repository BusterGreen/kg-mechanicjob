        ...
        while true do
            ...
            if isDriver then
                if IsControlJustPressed(0, 155) then
                    ...
                    TriggerServerEvent('kg-mechanicjob:server:syncNitrousFlames', netId, true)
                    nitrousActive = true
                end
                ...
                if IsControlJustReleased(0, 155) or nitrousVehicles[plate].level <= 0 then
                    ...
                    TriggerServerEvent('kg-mechanicjob:server:syncNitrousFlames', netId, false)
                    if nitrousVehicles[plate].level <= 0 then
                        nitrousVehicles[plate].hasnitro = false
                        TriggerServerEvent('kg-mechanicjob:server:syncNitrous', plate, false)
                        ...
                        TriggerServerEvent('kg-mechanicjob:server:syncNitrous', plate, true, nitrousVehicles[plate].level)
                    end
                end
            else
                plate = nil
...
AddEventHandler('gameEventTriggered', function(event)
    if event == 'CEventNetworkPlayerEnteredVehicle' then
        ...
        plate = KGCore.Functions.GetPlate(vehicle)
        netId = NetworkGetNetworkIdFromEntity(vehicle)
        KGCore.Functions.TriggerCallback('kg-mechanicjob:server:getnitrousVehicles', function(vehs)
...
RegisterNetEvent('kg-mechanicjob:client:syncNitrousFlames', function(net, toggle)
...
RegisterNetEvent('kg-mechanicjob:client:installNitrous', function()
    if IsPedInAnyVehicle(PlayerPedId(), false) then return end
    local closestVehicle, distance = KGCore.Functions.GetClosestVehicle()
    ...
    local vehiclePlate = KGCore.Functions.GetPlate(closestVehicle)
    if not vehiclePlate then return end
    if not IsNearBone(closestVehicle, 'engine') then return end
    ToggleHood(closestVehicle)
    KGCore.Functions.Progressbar('use_nos', Lang:t('progress.nitrous'), 5000, false, true, {
        ...
        ToggleHood(closestVehicle)
        TriggerServerEvent('kg-mechanicjob:server:removeItem', 'nitrous')
        TriggerServerEvent('kg-mechanicjob:server:syncNitrous', vehiclePlate, true, 100)
