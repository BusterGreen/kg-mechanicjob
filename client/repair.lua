...
local function RepairPart(vehicle, plate, component)
    ...
    local hasItems = exports['kg-inventory']:HasItem(materials)
    if not hasItems then
        KGCore.Functions.Notify(Lang:t('warning.no_materials'), 'error')
        return
    end
    local componentLabel = Config.WearableParts[component].label or component
    ToggleHood(vehicle)
    KGCore.Functions.Progressbar('repairing_vehicle', string.format(Lang:t('progress.repairing'), componentLabel), 10000, false, true, {
        ...
        ToggleHood(vehicle)
        KGCore.Functions.Notify(string.format(Lang:t('success.part_repaired'), componentLabel), 'success')
        for item, amount in pairs(materials) do
            TriggerServerEvent('kg-mechanicjob:server:removeItem', item, amount)
        end
        TriggerServerEvent('kg-mechanicjob:server:repairVehicleComponent', plate, component)
    end, function()
        ToggleHood(vehicle)
    end)
end
...
RegisterNetEvent('kg-mechanicjob:client:PartsMenu', function()
    local vehicle, distance = KGCore.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    if not IsNearBone(vehicle, 'engine') then return end
    local plate = KGCore.Functions.GetPlate(vehicle)
    if not plate then return end
    local vehicleClass = GetVehicleClass(vehicle)
    if Config.IgnoreClasses[vehicleClass] then return end
    KGCore.Functions.TriggerCallback('kg-mechanicjob:server:getVehicleStatus', function(status)
...
RegisterNetEvent('kg-mechanicjob:client:repairVehicle', function()
    local vehicle, distance = KGCore.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    if not IsNearBone(vehicle, 'engine') then return end
    local engineHealth = GetVehicleEngineHealth(vehicle)
    local bodyHealth = GetVehicleBodyHealth(vehicle)
    ToggleHood(vehicle)
    KGCore.Functions.Progressbar('repairing_vehicle', Lang:t('progress.repair_vehicle'), 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ToggleHood(vehicle)
        KGCore.Functions.Notify(Lang:t('success.repaired'), 'success')
        TriggerServerEvent('kg-mechanicjob:server:removeItem', 'repairkit')
    end, function()
        ToggleHood(vehicle)
    end)
end)

RegisterNetEvent('kg-mechanicjob:client:repairVehicleFull', function()
    local vehicle, distance = KGCore.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    if not IsNearBone(vehicle, 'engine') then return end
    ToggleHood(vehicle)
    KGCore.Functions.Progressbar('repairing_vehicle', Lang:t('progress.repair_vehicle'), 10000, false, true, {
        ...
        ToggleHood(vehicle)
        KGCore.Functions.Notify(Lang:t('success.repaired'), 'success')
        TriggerServerEvent('kg-mechanicjob:server:removeItem', 'advancedrepairkit')
    end, function()
        ToggleHood(vehicle)
    end)
end)

RegisterNetEvent('kg-mechanicjob:client:repairTire', function()
    local vehicle, distance = KGCore.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    local closestWheel = GetClosestWheel(vehicle)
    if not closestWheel then return end
    KGCore.Functions.Progressbar('repairing_tire', Lang:t('progress.repair_tire'), 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        KGCore.Functions.Notify(Lang:t('success.tire_repaired'), 'success')
        TriggerServerEvent('kg-mechanicjob:server:removeItem', 'tirerepairkit')
    end)
end)

RegisterNetEvent('kg-mechanicjob:client:cleanVehicle', function()
    local vehicle, distance = KGCore.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    KGCore.Functions.Progressbar('cleaning_vehicle', Lang:t('progress.cleaning'), 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        KGCore.Functions.Notify(Lang:t('success.cleaned'), 'success')
        TriggerServerEvent('kg-mechanicjob:server:removeItem', 'cleaningkit')
    end)
end)

RegisterNetEvent('kg-mechanicjob:client:fixEverything', function()
    KGCore.Functions.TriggerCallback('kg-mechanicjob:server:hasPermission', function(hasPermission)
