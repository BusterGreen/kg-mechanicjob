...
local function PaintList(category)
    ...
    if dialog.primarypaint and dialog.secondarypaint and dialog.pearlescentpaint and dialog.wheelpaint then
        ...
        TriggerServerEvent('kg-mechanicjob:server:sprayVehicle', netId, dialog.primarypaint, dialog.secondarypaint, dialog.pearlescentpaint, dialog.wheelpaint, colors)
    end
end
...
local function CustomColor()
    ...
    if (dialog.hex or dialog.colorpicker) and dialog.section then
        ...
        local netId = NetworkGetNetworkIdFromEntity(vehicle)
        if isPainting then return end
        TriggerServerEvent('kg-mechanicjob:server:sprayVehicleCustom', netId, dialog.section, dialog.paintType, HexToRGB(color))
    end
end
...
function PaintCategories()
    ...
    exports['kg-menu']:openMenu(Paints)
end
...
local function WindowTint(vehicle)
    ...
    exports['kg-menu']:openMenu(tints)
end
...
local function PlateIndex(vehicle)
    ...
    exports['kg-menu']:openMenu(plates)
end

-- Events

RegisterNetEvent('kg-mechanicjob:client:vehicleSetColors', function(netId, section, colorIndex)
    ...
    local props = KGCore.Functions.GetVehicleProperties(vehicle)
    TriggerServerEvent('kg-mechanicjob:server:SaveVehicleProps', props)
end)

RegisterNetEvent('kg-mechanicjob:client:startParticles', function(netId, color)
    isPainting = true
    local vehicle = NetworkGetEntityFromNetworkId(netId)
...
RegisterNetEvent('kg-mechanicjob:client:stopParticles', function()
    isPainting = false
...
RegisterNetEvent('kg-mechanicjob:client:installCosmetic', function(item)
    local vehicle, distance = KGCore.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    ...
    if item == 'veh_interior' then
        OpenInteriors(vehicle)
        TriggerServerEvent('kg-mechanicjob:server:removeItem', item)
