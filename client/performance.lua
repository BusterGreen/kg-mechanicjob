local function GetArmor(vehicle)
    ...
    for i = -1, GetNumVehicleMods(vehicle, 16) - 1 do
        ...
        local armorItem = {
            ...
            disabled = disabled,
            params = {
                event = 'kg-mechanicjob:client:install',
                args = {
                    upgradeType = 'armor',
                    modType = 16,
                    upgradeIndex = i,
    ...
    exports['kg-menu']:openMenu(armorMenu, true)
end
...
local function GetBrakes(vehicle)
    ...
    exports['kg-menu']:openMenu(brakesMenu, true)
end
...
local function GetEngine(vehicle)
    ...
    exports['kg-menu']:openMenu(engineMenu, true)
end
...
local function GetSuspension(vehicle)
    ...
    exports['kg-menu']:openMenu(suspensionMenu, true)
end
...
local function GetTransmission(vehicle)
    ...
    exports['kg-menu']:openMenu(transmissionMenu, true)
end
...
local function GetTurbo(vehicle)
    ...
    exports['kg-menu']:openMenu(turboMenu, true)
end

-- Events

RegisterNetEvent('kg-mechanicjob:client:install', function(data)
    ...
    KGCore.Functions.Progressbar('mechanic_install', string.format(Lang:t('progress.installing'), partName), 5000, false, true, {
        ...
        TriggerServerEvent('kg-mechanicjob:server:removeItem', 'veh_' .. partName)
        KGCore.Functions.Notify(string.format(Lang:t('success.installed'), partName), 'success')
    end, function()
        if shouldToggleHood then ToggleHood(data.vehicle) end
    end)
end)

RegisterNetEvent('kg-mechanicjob:client:installPart', function(item)
